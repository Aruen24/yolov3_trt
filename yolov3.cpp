...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::ofstream p("yolov3.trt", std::ios::binary);
        if (!p) {
        ...
        std::ifstream file("yolov3.trt", std::ios::binary);
        if (file.good()) {
    ...
    assert(engine != nullptr);
    //创建一个context用来存储中间激活值
    IExecutionContext* context = engine->createExecutionContext();
    assert(context != nullptr);
    delete[] trtModelStream;
    
    std::cout << "NetWork Width Height: " << INPUT_W<<"  " << INPUT_H << std::endl;
    int fcount = 0;
